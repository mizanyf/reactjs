...
const Home = () => {
  const team = [
    { name: 'Mizan', path: '/mizan', role: 'Team Leader', color: 'bg-blue-600' },
    { name: 'Aditya', path: '/aditya', role: 'Developer', color: 'bg-emerald-600' },
    { name: 'Ali', path: '/ali', role: 'Developer', color: 'bg-purple-600' },
  ];
        ...
        {team.map((member) => (
          <Link to={member.path} key={member.path} className="group">
            <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-slate-100 hover:shadow-xl hover:-translate-y-2 transition-all duration-300 flex flex-col items-center">
              <div className={`w-16 h-16 ${member.color} rounded-2xl mb-6 flex items-center justify-center text-white shadow-lg`}>
              ...
              <div className="mt-6 px-6 py-2 bg-slate-50 text-slate-400 group-hover:bg-blue-600 group-hover:text-white rounded-full text-xs font-bold transition-all">
                Lihat Profil →
              </div>
            </div>
          </Link>
